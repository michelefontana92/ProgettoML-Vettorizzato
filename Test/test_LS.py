...
if __name__ == '__main__':
    ...
    X = P[:, : - 2]
    T = P[:, -2:]
    trainer = TrainBackPropLS(eta_start=0.1,eta_max=2,max_iter=100,m1=0.001,m2=0.9,tau=0.7,sfgrd=0.001,mina=1e-16)
    mlp = MLP(n_features, n_hidden, n_out, TanhActivation(), LinearActivation(), lambd=lambd, eta=eta, alfa=alpha,trainer=trainer)

    #train(self, mlp, X, T, X_val, T_val, n_epochs=1000, eps=1e-6, threshold=0.5, suppress_print=False):
    mlp.trainer.train(mlp,addBias(X),T,addBias(X),T,n_epochs=5000)
